/**
 * Retoma CT-11 a partir do composer (boleto já anexado).
 * Uso: npx tsx server/scripts/resume-ct11-boleto.ts
 */
...
const RESUME =
  "projects/polygonus/automation/maestro/flows/debug/resume_ct11_boleto_from_composer.yaml";

console.log("[resume-ct11] Fase 1 restante — texto → Enviadas…");
const phase1 = await runMaestroFlow(RESUME, {
  onOutput: (c) => process.stdout.write(c),
  reinstallDriver: true,
});
if (!phase1.ok) {
  console.error("\n--- CT-11 resume: FALHOU na fase 1 ---\n");
  if (phase1.failure) console.error(JSON.stringify(phase1.failure, null, 2));
  process.exit(phase1.exitCode ?? 1);
}

console.log("[resume-ct11] Fase 2/3 — capturando ID (adb)…");
let id: string | null = null;
for (let i = 0; i < 4 && !id; i++) {
  try {
    id = captureMuralCardId(0);
  } catch (e) {
    console.warn(`[resume-ct11] adb tentativa ${i + 1}:`, e);
  }
  if (!id) await new Promise((r) => setTimeout(r, 2500));
}
if (!id) {
  console.error("[resume-ct11] ID ausente");
  process.exit(1);
}
const idDigits = id.replace(/[^0-9]/g, "");
console.log(`[resume-ct11] ID capturado: ${id}`);
...
try {
  ...
  console.log(
    `\n--- CT-11 resume: ${phase3.ok ? "OK" : "FALHOU"} (exit ${phase3.exitCode}) ---\n`,
  );
