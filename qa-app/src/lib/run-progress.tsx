...
export function RunProgressProvider({ children }: { children: ReactNode }) {
  ...
  const stopRun = useCallback(async () => {
    ...
    publishRunState({
      ...
      lines: [...s.lines.slice(-119), "[qa-app] Solicitando parada…"].filter(Boolean),
    });
    ...
    try {
      if (s.project) {
        ...
        finishCancel(
          "[qa-app] Nenhum processo Maestro ativo (prep/adb) — cancelamento registrado.",
        );
        return;
      }
    } catch {
    ...
    finishCancel("[qa-app] Parada local (servidor indisponível).");
  }, []);
    ...
    const timeout = window.setTimeout(() => {
      ...
      publishRunState({
        ...
        lines: [
          ...
          "[qa-app] Parada forçada no painel (servidor não respondeu a tempo).",
        ].filter(Boolean),
      });
      signalRunComplete({
        ...runState,
