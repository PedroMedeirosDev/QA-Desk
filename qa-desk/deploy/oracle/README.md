...
O Caddyfile isola streams (`…/kb-curation/stream`, `/api/bugs/gestor-replies/stream` e `…/github-issue` / `…/github-issue/sync`) **sem gzip** e com `flush_interval -1`. Sem isso a UI só atualiza no F5.
