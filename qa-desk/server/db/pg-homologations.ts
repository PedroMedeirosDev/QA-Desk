...
export async function writeHomologationCatalogToDb(
  ...
  // Sequential writes — interactive $transaction breaks on Supabase Transaction pooler (PgBouncer).
  ...
  if (ids.length === 0) {
    await prisma.homologation.deleteMany({ where: { projectSlug: project } });
    return;
  }

  await prisma.homologation.deleteMany({
    where: { projectSlug: project, id: { notIn: ids } },
  });

  for (const h of catalog.homologations) {
    const data = homologationToRow(h, project);
    await prisma.homologation.upsert({
      where: { id: h.id },
      create: data,
      update: data,
    });
