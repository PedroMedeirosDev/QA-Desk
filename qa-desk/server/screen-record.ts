...
export function startAdbScreenRecord(opts: {
  ...
  const runChunk = (index: number): Promise<void> =>
    new Promise((resolve) => {
      ...
      log(`[qa-desk] screenrecord chunk ${index}/${maxChunks} → ${remote}`);
  ...
  loopDone = (async () => {
    ...
    while (active && chunkIndex < maxChunks) {
      ...
      if (chunkIndex >= maxChunks) {
        log(
          `[qa-desk] screenrecord: limite de ${maxChunks} chunks (~${(maxChunks * CHUNK_SECONDS) / 60} min) atingido`,
        );
        break;
      }
    }
  ...
  return {
    async stop() {
      ...
      log(`[qa-desk] screenrecord fim: ${localPaths.length} arquivo(s)`);
